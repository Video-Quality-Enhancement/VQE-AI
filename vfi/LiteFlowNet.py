...
try:
    from vfi.correlation import correlation # the custom cost volume layer
except:
    sys.path.insert(0, './vfi/correlation'); import correlation # you should consider upgrading python
# end
...
class FlowNet(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load('model_weights/vfi/liteflownet-sintel.pytorch').items() })
    # end
